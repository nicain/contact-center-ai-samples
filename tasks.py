...
with open('.github/workflows/linter.yaml', 'r') as file:
...
@task
def lint(c, linter='all', warn=False, hide=False):
  if linter == 'all':
    any_failure = False
    for linter in _LINTER_CONFIG:
      result = lint(c, linter, warn=True, hide=True)
